...
well-known roles the UI understands (time, position, speed, rpm, ...) using
name heuristics, so most files visualize with zero configuration. The
detected mapping is returned to the client, which lets the user override it.
"""
...
ROLE_PATTERNS: dict[str, list[str]] = {
    ...
    "gear": [r"^(gear|gear_pos|current_gear|gear_position|shift)$", r"gear"],
    "throttle": [
    ...
    "steering": [
        r"^(steer|steering|steering_angle|steer_angle|swa|handle|steering_wheel_angle)$",
        r"steer",
    ],
    "accel_x": [r"^(ax|acc_x|accel_x|acceleration_x|long_accel|longitudinal_accel|imu_ax|a_long)$"],
    "accel_y": [r"^(ay|acc_y|accel_y|acceleration_y|lat_accel|lateral_accel|imu_ay|a_lat)$"],
    "accel_z": [r"^(az|acc_z|accel_z|acceleration_z|vert_accel|imu_az)$"],
    "yaw": [r"^(yaw|heading|course|bearing|azimuth|psi|yaw_angle)$", r"heading"],
    "yaw_rate": [r"^(yaw_rate|yawrate|r|gyro_z|omega|angular_velocity_z)$"],
    "altitude": [r"^(alt|altitude|elevation|height|gps_alt)$"],
}
...
def guess_unit(column: str, role: Optional[str]) -> str:
    n = re.sub(r"[\s\-\.\[\]\(\)/]+", "_", column.strip().lower())
