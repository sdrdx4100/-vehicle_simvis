...
- **ETAS INCA / A2L 信号の自動判別** — ECU 内部ラベル（`nmot`＝回転数・`vfzg`＝車速・
  `wped`＝アクセル・`gang`＝ギア・`along`/`aquer`＝前後/横 G …の Bosch 系ニーモニック）を
  辞書照合。`Device\Signal\10ms` のデバイス接頭辞・ラスタ接尾辞・`.value` も自動で剥がします。
  A2L 名は標準ではなくプロジェクト依存なのでベストエフォート判定ですが、外したものは
  UI の信号マッピングセレクタで手動指定できます（辞書は `simvis/inca.py` で拡張可）。
- **ゼロコンフィグ読み込み** — 上記以外のログも、列名からチャンネル役割（時刻 / GPS /
...
- **車両サイドビュー** — 手描き SVG のトラックが再生に連動して「走り」ます:
  車輪回転＝積算走行距離、路面の白線スクロール＝車速、車体ピッチ＝前後加速度
  （加速でノーズアップ、制動でノーズダイブ）。速度チャンネルさえあれば動くので
  GPS の無い CAN ログでも楽しめます。さらに **GPS 標高から路面勾配（%）を算出**し、
  シーン全体を傾けてトラックが坂を登り降りする様子を再現します（距離窓 ±25 m で
  平滑化して GPS 標高ノイズを除去、勾配バッジを表示）。
- **トラックマップ** — GPS（lat/lon）またはローカル XY から走行軌跡を描画。
  位置チャンネルが無いログではマップカードを自動的に畳み、他のパネルに領域を
  譲ります（巨大な空白は出ません）。
